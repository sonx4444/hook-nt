...
static bool IsValidApiName(const BYTE* name, size_t length) {
    if (length == 0 || length > TRACE_MAX_API_NAME_BYTES) {
        return false;
    }
...
static bool ParseTraceEvent(
    ...
    if (!IsValidApiName(cursor, event.header.apiNameLength)) {
        return false;
    }
    if (apiName) {
...
static size_t TextLabelWidth(const std::vector<TraceFieldView>& fields, const TraceEvent& event) {
    size_t width = strlen("dropped_before");
...
bool IsValidTraceEvent(const TraceEvent& event, size_t bytesReceived) {
    return ParseTraceEvent(event, bytesReceived, nullptr, nullptr);
}

void RenderTraceEventText(std::ostream& output, const TraceEvent& event) {
    std::string apiName;
    std::vector<TraceFieldView> fields;
    if (!ParseTraceEvent(event, event.header.size, &apiName, &fields)) {
        return;
    }
    ...
    RenderTextLabel(output, "dropped_before", labelWidth);
    output << event.header.droppedBefore << "\n";
    for (const TraceFieldView& field : fields) {
    ...
    RenderTextLabel(output, "result", labelWidth);
    output << HexStatus(event.header.status) << "\n";
    output.flush();
}
...
void RenderTraceEventJsonl(std::ostream& output, const TraceEvent& event) {
    ...
    output << "{\"sequence\":" << event.header.sequence
           << ",\"dropped_before\":" << event.header.droppedBefore
           ...
           << ",\"status\":" << (int32_t)event.header.status
           << ",\"status_hex\":\"" << HexStatus(event.header.status) << "\""
