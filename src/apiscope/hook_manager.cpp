...
bool HookFunction(
    ...
    if (!originalFunction) {
        fprintf(stderr, "[!] Missing resolved address for %s\n", definition.canonicalName.c_str());
        return false;
    }
    fprintf(
        stderr,
    ...
    std::optional<RemoteExport> handlerExport = GetRemoteExport(
        hProcess,
        (PBYTE)hookImageBase,
        definition.handlerExport.c_str());
    if (!handlerExport || !handlerExport->address) {
        fprintf(stderr, "[!] Not found %s\n", definition.handlerExport.c_str());
        return false;
    }
    PVOID hookedFunction = handlerExport->address;
    fprintf(stderr, "[+] Found %s at 0x%p\n", definition.handlerExport.c_str(), hookedFunction);

    std::optional<RemoteExport> trampolineExport = GetRemoteExport(
        hProcess,
        (PBYTE)hookImageBase,
        definition.trampolineExport.c_str());
    if (!trampolineExport || !trampolineExport->address) {
        fprintf(stderr, "[!] Not found %s\n", definition.trampolineExport.c_str());
        return false;
    }
    PVOID trampolineVarAddr = trampolineExport->address;
    ...
    InstalledHook hook = {};
    hook.canonicalName = definition.canonicalName;
    ...
    // Patch the original export to jump to the hook handler.
...
bool ReleaseUnloadedHook(HANDLE hProcess, const InstalledHook& installedHook) {
    PVOID emptyTrampoline = nullptr;
    bool cleared = WriteProcessMemory(
        hProcess,
        installedHook.trampolineSlot,
        &emptyTrampoline,
        sizeof(emptyTrampoline),
        nullptr) != FALSE;
    bool freed = VirtualFreeEx(
        hProcess,
        installedHook.trampolineAddress,
        0,
        MEM_RELEASE) != FALSE;
...
size_t CalculatePatchSize(HANDLE hProcess, PVOID address) {
    ...
    while (totalSize < PATCH_SIZE && totalSize < bytesRead) {
        DecodedInstruction instruction = {};
        if (!DecodeInstruction64(
                buffer + totalSize,
                bytesRead - totalSize,
                &instruction) ||
            instruction.length == 0) {
            fprintf(stderr, "[!] Undecodable instruction in trampoline\n");
            return 0;
        }
        if (instruction.hasRipRelativeMemory) {
            fprintf(stderr, "[!] Unsupported RIP-relative instruction in trampoline\n");
            return 0;
        }
        if (instruction.hasRelativeImmediate) {
    ...
    fprintf(
        stderr,
        "[+] Calculated patch size: %zu bytes for %zu instructions\n",
        totalSize,
        instructionCount);
    return totalSize;
}
...
bool CreateBypassTrampolineForFunction(
    ...
    std::optional<RemoteExport> remoteExport =
        GetRemoteExport(hProcess, (PBYTE)moduleBase, functionName);
    if (!remoteExport || !remoteExport->address) {
        fprintf(stderr, "[!] Not found %s\n", functionName);
        return false;
    }
    PVOID originalFunction = remoteExport->address;
