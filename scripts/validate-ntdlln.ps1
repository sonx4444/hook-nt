...
if ($imports -match "Section contains the following imports:") {
    throw "ntdlln.dll must not contain imports"
}
...
if (-not $entryPoint -or $entryPoint -notmatch "^\s*0+\s+entry point") {
    throw "ntdlln.dll must have a zero entry point"
}
...
if ($tlsDirectory -and $tlsDirectory -notmatch "^\s*0+\s+\[\s*0+\]") {
    throw "ntdlln.dll must not contain TLS initialization"
}

Write-Host "Validated import-free ntdlln.dll"
