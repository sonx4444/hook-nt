...
int main() {
    ...
    BYTE registerRelativeInstructions[32];
    CustomMemSet(registerRelativeInstructions, 0x90, sizeof(registerRelativeInstructions));
    registerRelativeInstructions[0] = 0x48;
    registerRelativeInstructions[1] = 0x8B;
    registerRelativeInstructions[2] = 0x00;
    if (CalculatePatchSize(GetCurrentProcess(), registerRelativeInstructions) != PATCH_SIZE) {
        printf("Register-relative memory instruction was not accepted\n");
        return 1;
    }
    ...
    BYTE undecodableInstructions[32];
    CustomMemSet(undecodableInstructions, 0x66, sizeof(undecodableInstructions));
    if (CalculatePatchSize(GetCurrentProcess(), undecodableInstructions) != 0) {
        printf("Undecodable instruction stream was not rejected\n");
        return 1;
    }
